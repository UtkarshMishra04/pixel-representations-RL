...
class PixelSacAgent(BaseSacAgent):
    ...
    def __init__(
        ...
        print('########################################################################')
        print('############## Starting Case 8: Baseline + Reconstruction ##############')
        print('##################### Reward: Yes; Transition: Yes #####################')
        print('################# Reconstruction, Reward and Transition ################')
        print('########################################################################')
        ...
        if self.encoder_type == 'pixel':
            ...
            self.obs_dec_fc = nn.Linear(encoder_feature_dim, 32 * 9 * 9)
            self.obs_dec = mutils.conv_mlp_decoder(obs_shape, encoder_feature_dim, 2)
    ...
    def update_transition_model(self, image_obs, obs, action, targets, rewards):
        
        transition_next_states, _ = self.transition_model.forward(obs, action[:-1])
        
        predicted_batch_rewards = []
        predicted_decoded_images = []
        for en_iter in range(transition_next_states.size(0)):
            ...
            decoded_image = self.obs_dec(self.obs_dec_fc(transition_next_states[en_iter]).view(-1, 32, 9, 9))
            predicted_decoded_images.append(decoded_image)

        predicted_batch_rewards = torch.stack(predicted_batch_rewards).squeeze()
        predicted_decoded_images = torch.stack(predicted_decoded_images).squeeze()
        
        loss = nn.MSELoss()(transition_next_states, targets)
        obs_decode_loss = nn.MSELoss()(predicted_decoded_images, image_obs.squeeze())
    ...
    def update(self, replay_buffer, L, step):
        if self.encoder_type == 'pixel':
            ...
            transition_loss, reward_loss, obs_decoder_loss = self.update_transition_model(batch_obs[1:], input_encoded_obs, batch_action, target_encoded_obs, target_rewards)
        ...
        total_loss = transition_loss + reward_loss + obs_decoder_loss

        self.encoder_optimizer.zero_grad()
        self.decoder_optimizer.zero_grad()
        self.obs_decoder_optimizer.zero_grad()
        total_loss.backward()
        self.encoder_optimizer.step()
        self.decoder_optimizer.step()
        self.obs_decoder_optimizer.step()
