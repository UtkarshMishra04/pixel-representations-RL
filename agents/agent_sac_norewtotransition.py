...
class PixelSacAgent(BaseSacAgent):
    ...
    def __init__(
        ...
        print('########################################################################')
        print('################### Starting Case 4: Baseline Agent ####################')
        print('#################### Reward: Yes; Transition: Yes ######################')
        print('################## Independent Reward and Transition ###################')
        print('########################################################################')
        ...
        if self.encoder_type == 'pixel':
            ...
            self.decoder_optimizer1 = torch.optim.Adam(
                self.reward_decoder.parameters(),
                lr=decoder_lr/5,
                weight_decay=decoder_weight_lambda
            )
            self.decoder_optimizer2 = torch.optim.Adam(
    ...
    def update_transition_model(self, obs, action, targets, rewards):
        ...
        for en_iter in range(targets.size(0)):
            pred_reward = self.reward_decoder(targets[en_iter])
